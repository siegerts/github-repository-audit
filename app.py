...
repo_meta = gh.get_repo(selected_org, selected_repo_name)
repo_name = repo_meta["name"].values[0]
...
try:
    repo_license = repo_meta["license.name"].values[0]
...
health = gh.get_repo_health(selected_org, selected_repo_name)
...
topics = sorted(
    gh.get_repo_topics(selected_org, selected_repo_name).values[0].tolist()[0]
)
...
if missing_topics:
    topic_suggestions = (
        f"The project is missing the following topic(s): {(', ').join(missing_topics)}"
    )
...
labels = gh.get_repo_labels(selected_org, selected_repo_name)[
...
st.write(
    """
These are the common labels required across all Amplify repositories.
...
for i, label in enumerate(req_labels):
    ...
    if idx:
        ...
        if color_flag:
            notes += (
                f"Color needs changed to **{required_labels[i]['color'].lower()}**. "
            )
        desc_flag = labels.values[idx][2] != required_labels[i]["description"]
        if desc_flag:
            notes += (
                f"Description needs changed to **{required_labels[i]['description']}**."
            )
...
|-------|--------|-------|-------------|---------|\n"""
    ...
    for label in missing_labels:
        similar = [
            f"`{l}`"
            for l in difflib.get_close_matches(label["label"], repo_labels, cutoff=0.8)
        ]
...
if deprecated:
    ...
    st.warning(
        "🚨 The following required labels are deprecated. **Don't delete!** Instead, transition away from use."
    )
...
[View the {selected_repo_name} GitHub Contributor page](https://github.com/{selected_org}/{selected_repo_name}/contribute).

Is it welcoming to a new contributor to the project?

"""
)
contrib_labels = gh.get_open_for_contrib_issues(selected_org, selected_repo_name)
...
for f in codeowners:
    fp = gh.get_repo_files(selected_org, selected_repo_name, f)
    if fp:
...
required_forms = ["issue", "bug", "feature"]
